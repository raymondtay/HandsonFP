...
object MyTransformation {
  ...
  def isSorted[A](xs: Array[A], ordered: (A, A) => Boolean) : Boolean = {
    if (xs.isEmpty) false else
    if (xs.length == 1) true else
    if (xs.length == 2) ordered(xs.head, xs.tail.head) else {
      val head = xs.head
      val rem  = xs.tail
      val r = rem.tail.foldLeft(ordered(head, rem.head))((acc, e) => acc && ordered(rem.head,e))
      r == true
    }
  }
